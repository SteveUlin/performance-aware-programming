...
std::string GenAsmRegisterToFromRegister(const std::string_view op,
  ...
  if (d)
    return fmt::format("{} {}, {}", op, reg_name, rm_name);
...
std::string GenAsmRegisterToFromEffectiveAddress(const std::string_view op,
  ...
  if (d)
    return fmt::format("{} {}, [{}{}]", op, reg_name, ea, offset);
...
std::string GenAsmRegisterToFromDirectAddress(const std::string_view op,
  ...
  std::string suffix = "";
  if (data)
    suffix = fmt::format(" + {}", data);
  if (d)
    return fmt::format("{} {}, [{}]{}", op, reg_name, disp, suffix);
...
public:
  static bool TryParse(u8 *&head, const u8 *const end, std::string &asm_str) {
    ...
    if (head + 2 + inst->mod_ > end)
      return false;
    u16 disp = 0;
    if (inst->mod_ >= 0b01)
      disp = static_cast<u16>(*(head + 2));
    if (inst->mod_ == 0b10)
      disp |= static_cast<u16>(*(head + 3)) << 8;
...
private:
  std::string GenAsm() const {
    ...
    if (w_)
      return fmt::format("mov {}, {}", reg,
                         static_cast<u16>(data0_) |
                             (static_cast<u16>(data1_) << 8));
...
public:
  static bool TryParse(u8 *&head, const u8 *const end, std::string &asm_str) {
    ...
    if (inst->prefix_ != 0b00 || inst->suffix_ != 0b10)
      return false;
    if (inst->w_ == 1 && head + 3 > end)
      return false;
    u16 data = *(head + 1);
    if (inst->w_)
      data |= (static_cast<u16>(*(head + 2)) << 8);
